...
def create_feflow_prismatic(name, geo_model, units_data, points_dom, points, constraints,
    ...
    nump = len(vertices)*(layers + 1)
    ne = len(triangles)*layers
    
    first  = "%(nump)6d %(ne)6d %(nbn)6d %(numb_dt)6d " % {"nump": nump, "ne": ne, "nbn": 6, "numb_dt": 1}
    second = "%(icrank)6d %(upwind)6d %(obs)6d %(optim)6d %(phreatic)6d " % {"icrank": 0, "upwind": 0, "obs": 0, "optim": 0, "phreatic": 0}
    third  = "%(nwca)6d %(npcor)6d %(adaptive_mesh)6d %(special_fem_process_id)6d " % {"nwca": 2, "npcor": 0, "adaptive_mesh": 0, "special_fem_process_id": 0}
    fourth = "%(sorption_type)6d %(reaction_type)6d %(dispersion_type)6d " % {"sorption_type": 0, "reaction_type": 0, "dispersion_type": 0}
    fifth  = "%(reaction_popt)6d %(dual_nodes)6d " % {"reaction_popt": 0, "dual_nodes": 0}
    ...
    # NODE
    nump_lay = len(vertices)
    fd.write("NODE\n")
    for l in xrange(layers):
        for elem in triangles:
            Elem = (l*nump_lay + elem).tolist() + ((l+1)*nump_lay + elem).tolist()
            fd.write( "".join( map(node_str, Elem) ) )
            fd.write( "\n" )
    ...
    fd.write("ELEV_I\n")
    conv_range = lambda r: str(r[0]+1) if r[0]==r[1] else "%s-%s" % (r[0]+1, r[1]+1)
    conver = lambda pair: (pair[0],int(pair[1]))
    add_line = lambda  strL : '\n'.join([" ".join([str(v) for v in strL[ i*num_columns :min((i+1)*num_columns, len(strL) )]])
        for i in range( int(ceil( len(strL)/(num_columns + 0.0) )) )])
    
    def write_slice( slice_values ):
        for Z, idxs in slice_values:
            f = "%21.14le %s\n" if Z > 0 else " %21.14le %s\n"
            fd.write(f % ( Z, " ".join(map(conv_range,to_range(idxs) ) )))
            # fd.write(f % ( Z, add_line(map(conv_range,to_range(idxs) ) )))
    ...
    for lay in bottoms:
        ...
        if l>1:
            fd.write( "%d\n" % l )    
        slice_values = map(conver,np.c_[lay,np.arange(nump_lay)])
        slice_values.sort()
        slice_values = redu_range( slice_values )
        write_slice(slice_values)
    ...
    for l in range(layers):
        for tri in triangles:
            z_elem = np.c_[bottoms[l,tri], bottoms[l+1,tri]]
            xy_point = np.mean(vertices[tri],0)
            pt_mean = [xy_point[0], xy_point[1], np.mean(z_elem)]
            unit = geo_model.closest( pt_mean )[0]

            if unit_elements.has_key(unit):
                unit_elements[unit].append(idx)
            else:
                unit_elements[unit] = [idx]
            idx+=1

    ranges = {}
    for u, indices in unit_elements.iteritems():
        ranges[u] = to_range(indices)
    ...
    for i in range(4):
        ...
        for v in materials:
            ...
            while j < len(rngs):
                ...
                fd.write("\t\t%(node_list)s\n" % { "node_list": node_list } )
                j += num_columns
        # get_info = lambda pair: (units_data[pair[0]][i], pair[1])
        # materials = {}
        # for unit_pair in slice_unit:



        # slice_info = map(get_info, slice_unit)
        # slice_info.sort()
        # slice_info = redu_range( slice_info )
        # print slice_info[0]
        # exit(0)
        # write_slice(slice_info)
